...
const SERVICE_EVENTID_HINTS: Record<string, number[]> = {
  ...
  security: [
    ...
    4733, 4734, 4735, 4737, 4738, 4739, 4740, 4741, 4742, 4743,
    // Group management
  ...
  windefend: [
    1006, 1007, 1008, 1009, 1010, 1011, 1013, 1116, 1117, 1118, 1119, 5001,
    5007, 5010, 5012,
  ],
  ...
  "printservice-operational": [
    307, 800, 801, 805, 812, 823, 831, 842, 843, 845, 846, 847, 848, 849,
  ],
 ...
 */
export function matchesExpectedProvider(
  event: any,
...
export function isWindowsCompatibleRule(logsource: SigmaLogSource): boolean {
  ...
  if (
    logsource.product &&
    nonWindowsProducts.includes(logsource.product.toLowerCase())
  ) {
    return false;
  }
  ...
  if (
    logsource.service &&
    nonWindowsServices.includes(logsource.service.toLowerCase())
  ) {
    return false;
  }
 ...
 */
export function filterWindowsRules(
...
export function filterRulesByAvailableEventIDs(
  ...
  for (const rule of rules) {
    ...
    if (ruleEventIds === null) {
      ...
      const hasMatchingEventId = ruleEventIds.some((id) =>
...
/**
 * Infer likely Linux event source types for a rule using logsource service hints.
 * Returns null when a rule should run against all Linux events.
 */
function inferLinuxRuleSourceTypes(
  rule: CompiledSigmaRule,
): Set<string> | null {
  const service = rule.rule.logsource?.service?.toLowerCase() || "";
  if (!service) return null;

  // auditd-only services
  if (service.includes("audit")) {
    return new Set(["auditd"]);
  }

  // journal/systemd-only services
  if (
    service.includes("journal") ||
    service.includes("systemd") ||
    service.includes("journald")
  ) {
    return new Set(["journal"]);
  }

  // Syslog-like services can appear in traditional syslog and journal exports
  if (
    service.includes("syslog") ||
    service.includes("auth") ||
    service.includes("secure") ||
    service.includes("sshd") ||
    service.includes("sudo") ||
    service.includes("cron")
  ) {
    return new Set(["syslog", "journal"]);
  }

  return null;
}

function buildLinuxSourceIndex(events: LogEntry[]): Map<string, LogEntry[]> {
  const index = new Map<string, LogEntry[]>();

  for (const event of events) {
    const sourceType = (event.sourceType || "unknown").toLowerCase();
    const bucket = index.get(sourceType);
    if (bucket) {
      bucket.push(event);
    } else {
      index.set(sourceType, [event]);
    }
  }
...
function gatherEventsForSourceTypes(
  sourceIndex: Map<string, LogEntry[]>,
  sourceTypes: Set<string>,
): LogEntry[] {
  const out: LogEntry[] = [];

  for (const sourceType of sourceTypes) {
    const events = sourceIndex.get(sourceType);
...
export function extractQuickFilters(rule: CompiledSigmaRule): QuickFilter[] {
  ...
  const priorityFields = [
    ...
    // File fields
    "TargetFilename",
    // Linux fields (auditd/journal/syslog)
    "exe",
    "comm",
    "cmdline",
    "proctitle",
...
function extractFieldForQuickCheck(event: LogEntry, field: string): string {
  ...
  // Common normalized fields for Linux entries.
  if (lowerField === "message") {
    return event.message || "";
  }
  if (lowerField === "hostname" || lowerField === "host") {
    return event.host || event.computer || "";
  }
  if (lowerField === "processname") {
    return event.processName || "";
  }
  if (lowerField === "processcmd" || lowerField === "commandline") {
    return event.processCmd || "";
  }
  ...
  // Case-insensitive lookup for Linux field names (e.g., MESSAGE vs message)
...
export async function processEventsOptimized(
  ...
  onProgress?: (
    processed: number,
    total: number,
    stats: Partial<OptimizedMatchStats>,
  ) => void,
  ...
  const isLinuxDataset = events.some(
    (event) => event.platform?.toLowerCase() === "linux",
  );

  // Step 2.5: Pre-filter rules by available EventIDs in EVTX datasets only.
  let filteredRules = rules;
  let rulesFilteredByEventID = 0;
  if (!isLinuxDataset) {
  ...
  const ruleGroups = isLinuxDataset
    ? [{ eventIds: null, rules: filteredRules }]
    : groupRulesByEventID(filteredRules);

  // Linux-specific grouping by source type hints to reduce comparisons.
  const linuxSourceIndex = isLinuxDataset
    ? buildLinuxSourceIndex(events)
    : new Map<string, LogEntry[]>();

  const linuxRuleGroups = new Map<string, CompiledSigmaRule[]>();
  if (isLinuxDataset) {
    for (const rule of filteredRules) {
      const sourceTypes = inferLinuxRuleSourceTypes(rule);
      const key = sourceTypes ? Array.from(sourceTypes).sort().join(",") : "*";
      const existing = linuxRuleGroups.get(key);
      if (existing) {
        existing.push(rule);
      } else {
        linuxRuleGroups.set(key, [rule]);
      }
    }
  }
  ...
  if (isLinuxDataset) {
    for (const [key, groupedRules] of linuxRuleGroups) {
      if (key === "*") {
        totalWorkUnits += events.length * groupedRules.length;
      } else {
        const sourceTypes = new Set(key.split(","));
        const targetEvents = gatherEventsForSourceTypes(
          linuxSourceIndex,
          sourceTypes,
        );
        totalWorkUnits += targetEvents.length * groupedRules.length;
      }
    }
  } else {
      ...
      for (const event of chunk) {
        ...
        for (const rule of groupRules) {
          ...
          if (
            ...
            !quickRejectCheck(event, quickFilters, fieldCache)
          ) {
            quickRejects++;
            completedWorkUnits++;
  ...
  if (isLinuxDataset) {
    for (const [key, groupedRules] of linuxRuleGroups) {
      const targetEvents =
        key === "*"
          ? events
          : gatherEventsForSourceTypes(
              linuxSourceIndex,
              new Set(key.split(",")),
            );

      if (targetEvents.length === 0) {
        rulesSkipped += groupedRules.length;
        continue;
      }

      await processGroup(targetEvents, groupedRules);
    }
  } else {
    ...
    for (const group of ruleGroups) {
      ...
      await processGroup(targetEvents, group.rules);
    }
  }
