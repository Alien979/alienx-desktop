 ...
 * Returns XML string compatible with ALIENX's existing XML parser
 *
...
export async function parseBinaryEVTXWithWasm(
  ...
  try {
    ...
    const totalRecords =
      toSafeNumber(fileInfo.total_records) ||
      toSafeNumber(fileInfo.next_record_id);
    ...
    console.log(
      `Parsing ${totalChunks} chunks in batches of ${CHUNKS_PER_BATCH}...`,
    );
    ...
    for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
      try {
        ...
        const safeChunkRecords = chunkRecords.map((rec: any) =>
          sanitizeBigInts(rec),
        );
        ...
        if (errorMsg.includes("can't be represented as a JavaScript number")) {
          console.warn(
            `Skipping chunk ${chunkIndex} due to BigInt timestamp issue`,
          );
          skippedChunks++;
        } else {
      ...
      if (
        (chunkIndex + 1) % CHUNKS_PER_BATCH === 0 ||
    ...
    console.log(
      `Successfully parsed ${allRecords.length} records from ${totalChunks} chunks`,
    );
    ...
    // Convert JSON records to XML format (to match existing ALIENX XML parser)
...
/**
 * Convert WASM JSON output to XML format expected by ALIENX's existing parser
 */
...
function buildEventDataXml(eventData: any): string {
  ...
  for (const [key, value] of Object.entries(eventData)) {
    ...
    dataItems.push(
      `    <Data Name="${escapeXml(key)}">${escapeXml(valueStr)}</Data>`,
    );
  }
    ...
    for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
      try {
        ...
        for (const rec of chunkRecords) {
          const entry = convertRecordToLogEntry(
            rec,
            filename,
            recordsProcessed,
          );
      ...
      if (
        onProgress &&
        ((chunkIndex + 1) % progressInterval === 0 ||
          chunkIndex === totalChunks - 1)
      ) {
 ...
 */
function convertRecordToLogEntry(
  record: any,
  filename?: string,
  sequence?: number,
): import("../types").LogEntry | null {
  try {
    // Convert Map to plain object if needed
...
function sanitizeField(value: any): string {
  ...
  if (typeof value === "object") {
    if ("#text" in (value as any))
      return sanitizeField((value as any)["#text"]);
