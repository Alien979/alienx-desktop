...
function findRelationships(
  ...
  entries.forEach((entry, idx) => {
    ...
    if (
      (entry.eventId === EVENT_TYPES.PROCESS_CREATE ||
        entry.eventId === EVENT_TYPES.SECURITY_PROCESS_CREATE) &&
      parentGuid
    ) {
    ...
    if (procGuid) {
      ...
      for (const otherIdx of sameProcessEvents) {
        if (otherIdx < idx) {
          // Only link to earlier events to avoid duplicates
          relationships.push({
    ...
    if (entry.eventId === EVENT_TYPES.NETWORK_CONNECT && procGuid) {
      ...
      for (const procIdx of processEvents) {
        if (
          procIdx !== idx &&
          (entries[procIdx].eventId === EVENT_TYPES.PROCESS_CREATE ||
            entries[procIdx].eventId === EVENT_TYPES.SECURITY_PROCESS_CREATE)
        ) {
          relationships.push({
            sourceIndex: procIdx,
            targetIndex: idx,
    ...
    if (
      [
        ...
        EVENT_TYPES.FILE_DELETE_LOGGED,
      ].includes(entry.eventId || 0) &&
      procGuid
    ) {
      const processEvents = indices.byProcessGuid.get(procGuid) || [];
      for (const procIdx of processEvents) {
    ...
    if (
      [
        ...
        EVENT_TYPES.REGISTRY_SET,
        EVENT_TYPES.REGISTRY_RENAME,
      ].includes(entry.eventId || 0) &&
      procGuid
    ) {
      const processEvents = indices.byProcessGuid.get(procGuid) || [];
...
function calculateSeverity(
  ...
  if (
    eventIds.some(
      (id) =>
        id === EVENT_TYPES.PROCESS_CREATE ||
        id === EVENT_TYPES.SECURITY_PROCESS_CREATE,
    ) &&
...
export function correlateEvents(
  ...
  // Cap at 200K events - ONLY events with SIGMA hits + context (±1 event)
  const MAX_EVENTS_TO_CORRELATE = 200000;
  ...
  if (eventsWithMatches.size === 0) {
    console.warn(
      "No SIGMA matches found. Event correlation requires SIGMA detections.",
    );
    return [];
  }
  ...
  if (eventsWithContext.size > MAX_EVENTS_TO_CORRELATE) {
    console.warn(
      `${eventsWithContext.size} events (SIGMA + context). Limiting to ${MAX_EVENTS_TO_CORRELATE}.`,
    );
    ...
    entriesToProcess = Array.from(finalEvents).sort(
      (a, b) => a.timestamp.getTime() - b.timestamp.getTime(),
    );
  } else {
    entriesToProcess = Array.from(eventsWithContext).sort(
      (a, b) => a.timestamp.getTime() - b.timestamp.getTime(),
    );
  }

  console.log(
    `Correlating ${entriesToProcess.length} events (${eventsWithMatches.size} SIGMA matches + context)`,
  );
  ...
  for (const [, eventIndices] of chainGroups) {
    ...
    eventIndices.sort(
      (a, b) =>
        entriesToProcess[a].timestamp.getTime() -
        entriesToProcess[b].timestamp.getTime(),
    );

    const chainEvents = eventIndices.map((i) => entriesToProcess[i]);
    const chainRelationships = relationships.filter(
      (r) =>
        eventIndices.includes(r.sourceIndex) &&
        eventIndices.includes(r.targetIndex),
    );
      ...
      for (const [, matches] of sigmaMatches) {
        for (const match of matches) {
          ...
          if (
            match.event?.rawLine &&
            event.rawLine &&
            match.event.rawLine === event.rawLine
          ) {
            matchedEventSet.add(match);
          }
        }
    ...
    const uniqueMatches = chainSigmaMatches.filter((match) =>
      chainEvents.some((event) => {
        ...
        if (
          event.rawLine &&
          matchEvent.rawLine &&
          event.rawLine === matchEvent.rawLine
        )
          return true;
  ...
  return {
    ...
    avgChainLength:
      chains.length > 0
        ...
        : 0,
    longestChain:
      chains.length > 0 ? Math.max(...chains.map((c) => c.events.length)) : 0,
