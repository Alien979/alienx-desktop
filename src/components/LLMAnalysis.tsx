...
import {
  ...
  loadConversation,
  clearConversation,
} from "../lib/llm/storage/conversations";
...
export default function LLMAnalysis({
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to analyze with LLM";
      ...
      if (
        ...
        userFriendlyError =
          "Rate limit exceeded. Please wait a moment and try again, or try a different provider.";
      } else if (
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Failed to analyze with file attachments";
  ...
  const handleExportResult = () => {
    if (analysisResult) {
      ...
      a.href = url;
      a.download = `alienx-analysis-${new Date().toISOString()}.txt`;
          ...
          {availableModels.length > 0 && (
            <>
              ...
              <div className="config-row">
                ...
                >
                  Larger values allow longer explanations but increase
                  cost/time. Try using at least 4000 tokens.
                </p>
              </div>
            </>
          )}
              ...
              <div className="followup-panel">
                      ...
                      if (
                        ...
                        document
                          .querySelector<HTMLButtonElement>(
                            ".followup-actions .analyze-button",
                          )
                          ?.click();
                      }
                    }
                ...
                <div className="followup-actions">
                  <button
                    className="analyze-button"
                    disabled={
                      isAnalyzing || !followUpInput.trim() || !hasApiKey
                    }
                    onClick={async () => {
                      ...
                      try {
                        const meta = getProviderMetadata(provider);
                        const apiKey = meta?.requiresApiKey
                          ? getAPIKey(provider)
                          : "";
                        if (meta?.requiresApiKey && !apiKey) {
                          setError(
                        ...
                        // For providers that require endpoint (like Ollama), get it from stored apiKey
                        const endpoint = meta?.requiresEndpoint
                          ? getAPIKey(provider)
                          : undefined;
                        ...
                        const response = await sendAnalysisRequest(
                          ...
                          lastFormattedPrompt?.systemPrompt ||
            ...
            <div className="empty-state">
              ...
              {!hasSigmaAnalysis ? (
                <>
                  ...
                  <p>
                    Before using AI analysis, you must first perform SIGMA
                    detection on your logs.
                  </p>
                  <p className="empty-note">
                    <strong>⚠️ Pre-requisite:</strong> Navigate to "SIGMA
                    Detection" from the Analysis Selector to scan your logs for
                    threats and suspicious patterns. Once SIGMA analysis is
                    complete, you can return here to get AI-powered insights.
                  </p>
                  <button
                    onClick={onBack}
                ...
                <>
                  ...
                  <p>
                    Configure your API key and click "Analyze with AI" to get
                    started.
                  </p>
                  <p className="empty-note">
                    The AI will analyze your SIGMA detections, timeline, and
                    event data to provide comprehensive security insights and
                    recommendations.
                  </p>
                </>
              )}
            ...
            <div className="prompt-preview-content">
              ...
              {provider === "openai" && (
                ...
                >
                  ...
                  >
                    All analysis data is sent as 3 PDF file attachments to
                    OpenAI Files API:
                  </p>
                  ...
                  >
                    ✓ Only the system prompt and a minimal user prompt are sent
                    as text. All data is in the attached files.
                  </p>
                </div>
              )}
              <div className="prompt-preview-actions">
                <button
                  onClick={() => {
                    ...
                    if (
                      provider === "openai" &&
                      (formattedPrompt as any).attachments
                    ) {
                      fullPrompt += `\n\n\nFILE ATTACHMENTS:\n\n`;
