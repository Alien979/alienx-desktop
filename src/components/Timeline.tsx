...
export default function Timeline({ matches, onBack }: TimelineProps) {
  ...
  const [selectedBucket, setSelectedBucket] = useState<AggregatedBucket | null>(
    null,
  );
  ...
  const timelineEvents = useMemo(() => {
    return allTimelineEvents.filter((event) => {
      ...
      if (filters.searchQuery) {
        ...
        const matchesSearch =
          event.rule.title.toLowerCase().includes(query) ||
      ...
      if (filters.computerFilter) {
        ...
        if (
          !computer.toLowerCase().includes(filters.computerFilter.toLowerCase())
        ) {
          return false;
        }
      }
  ...
  const aggregatedBuckets = useMemo(() => {
    ...
    for (const event of timelineEvents) {
      ...
      currentBucket.bySeverity[severity] =
        (currentBucket.bySeverity[severity] || 0) + 1;
    }
    ...
    <div className="timeline-page">
        ...
        <div>
          ...
          <p className="timeline-subtitle">
            ...
            {stats.timeRange && (
              <>
                {" "}
                • {stats.timeRange.start.toLocaleString()} to{" "}
      ...
      <div className="timeline-controls">
        ...
        >
          🔍 Filters{" "}
          {hasActiveFilters &&
            `(${allTimelineEvents.length - timelineEvents.length} hidden)`}
        </button>
      </div>
              ...
              <div
                ...
                onClick={() => {
                  if (selectedBucket === bucket) {
                    ...
                    setBucketVisibleCount(BUCKET_PAGE_SIZE);
                  }
                }}
              >
                <div className="timeline-bar-wrapper">
                  <div
                    ...
                    style={{
                      height: `${(bucket.events.length / maxEvents) * 100}%`,
                    }}
                  >
                ...
                <div className="timeline-label">
                  {formatBucketTime(bucket.startTime)}
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedBucket && (
        <div className="bucket-details">
          <div className="bucket-details-header">
            <h3>
              ...
              <span className="event-count">
                ({selectedBucket.events.length} events)
              </span>
            </h3>
            <button
              className="close-btn"
          ...
          <div className="bucket-events">
            {selectedBucket.events
              .slice(0, bucketVisibleCount)
              .map((event, idx) => (
                <div
                  key={idx}
                  className={`bucket-event ${expandedEvents.has(idx) ? "expanded" : ""}`}
                  style={{
                    borderLeftColor: getSeverityColor(event.rule.level),
                  }}
                >
                  <div
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        backgroundColor: getSeverityColor(event.rule.level),
                      }}
                    >
                  ...
                  {expandedEvents.has(idx) && (
                    <div className="event-details">
                      ...
                      <div className="detail-row">
                        ...
                        <span>
                          {event.event.eventId || event.event.EventID || "N/A"}
                        </span>
                      </div>
                      <div className="detail-row">
                        <span className="detail-label">Source:</span>
                        <span>
                          {event.event.source || event.event.Provider || "N/A"}
                        </span>
                      </div>
                      ...
                      {(() => {
                        ...
                        if (event.match.selectionMatches) {
                          for (const selMatch of event.match.selectionMatches) {
                            ...
                            let selectionDef;
                            if (
                              event.match.compiledRule &&
                              event.match.compiledRule.selections
                            ) {
                              const selection =
                                event.match.compiledRule.selections.get(
                                  selMatch.selection,
                                );
                        ...
                        return (
                          <div className="matched-fields">
                            <div className="matched-fields-header">
                              ...
                              >
                                (Tooltip hover shows only the exact values that
                                a condition matched against and NOT the entire
                                list)
                              </span>
                            </div>
            ...
            {selectedBucket.events.length > bucketVisibleCount && (
              <div
              ...
              >
                <span>
                  Showing {bucketVisibleCount} of {selectedBucket.events.length}{" "}
                  events
                </span>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <button
                    ...
                    onClick={() =>
                      setBucketVisibleCount((prev) => prev + BUCKET_PAGE_SIZE)
                    }
                  >
                    Load{" "}
                    {Math.min(
                  ...
                  <button
                    className="back-button"
                    style={{ fontSize: "0.85rem", padding: "0.3rem 0.8rem" }}
                    onClick={() => setBucketVisibleCount(Infinity)}
                  >
                    Show All ({selectedBucket.events.length})
                  </button>
                </div>
              </div>
            )}
