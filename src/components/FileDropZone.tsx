...
export default function FileDropZone({
  ...
  const [processingResults, setProcessingResults] =
    useState<MultiFileProcessingResults | null>(null);
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (isBinary) {
          ...
          setProcessingStatus(
            `Parsing binary EVTX file (${fileSizeMB.toFixed(1)} MB)...`,
          );
          ...
          const entries = await parseBinaryEVTXToEntries(
            ...
            (processed, total) => {
              ...
              setProcessingStatus(
                `${processed.toLocaleString()} / ${totalDisplay} records${percentage}`,
              );
            },
            file.name,
          );
  ...
  const aggregateResults = (
    ...
    const totalRecordsParsed = successfulFiles.reduce(
      (sum, r) => sum + (r.recordCount || 0),
      0,
    );
  ...
  const handleFiles = useCallback(
    async (files: CollectedFile[]) => {
          ...
          if (extractionResult.success && extractionResult.files.length > 0) {
            ...
            zipExtractionErrors.push({
              ...
              error: {
                type: ErrorType.INVALID_FORMAT,
                message:
                  extractionResult.error || "Failed to extract ZIP archive",
                technicalDetails:
                  extractionResult.error || "No EVTX or XML files found",
      ...
      for (let i = 0; i < filesToProcess.length; i++) {
        ...
        setProcessingStatus(
          `Processing file ${i + 1} of ${filesToProcess.length}: ${displayName}`,
        );
  ...
  const handleSampleSelect = useCallback(
    async (url: string, filename: string) => {
      ...
      try {
        ...
        alert(
          `Error loading sample: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        setIsProcessing(false);
      }
    },
  ...
  const handleMultipleSamples = useCallback(
    async (samples: Array<{ url: string; filename: string }>) => {
      ...
      for (let i = 0; i < samples.length; i++) {
        ...
        setCurrentFileIndex(i + 1);
        setProcessingStatus(
          `Fetching sample ${i + 1}/${samples.length}: ${filename}...`,
        );
        ...
        try {
          ...
          // Process the file
          setProcessingStatus(
            `Parsing ${i + 1}/${samples.length}: ${filename}...`,
          );
          const entries = await parseBinaryEVTXToEntries(
            file,
            (processed, total) => {
              ...
              const percentage =
                total > 0 ? ` (${Math.round((processed / total) * 100)}%)` : "";
              setProcessingStatus(
                `[${i + 1}/${samples.length}] ${filename}: ${processed.toLocaleString()} / ${totalDisplay}${percentage}`,
              );
            },
            filename,
          );
      ...
      {showProcessingResults && processingResults && (
        <FileProcessingResultsModal
          ...
          onProceed={(mergedData) => {
            ...
            onFileLoaded(
              mergedData,
              processingResults.successfulFiles
                .map((f) => f.filename)
                .join(", "),
            );
          }}
